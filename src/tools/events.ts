...
import { compactEvent, compactSeries, handle } from "../helpers.js";
...
export function registerEventTools(server: McpServer, client: KalshiClient): void {
  server.registerTool(
    "kalshi_list_events",
    {
      ...
      description:
        ...
        "Returns a compact summary per event by default; use kalshi_get_event for full details. " +
        "Responses are paginated; pass the returned `cursor` back in for the next page.",
      inputSchema: {
        limit: z.number().int().min(1).max(200).optional().describe("Results per page (default 25; keep small to avoid oversized responses)"),
        ...
        full_details: z
          .boolean()
          .optional()
          .describe("Return complete raw event objects instead of compact summaries (default false)"),
      },
    },
    handle(async (args: Record<string, string | number | boolean | undefined>) => {
      const { full_details, ...query } = args;
      const data = await client.request<{ events?: Record<string, unknown>[]; cursor?: string }>(
        "GET",
        "/events",
        { query: { ...query, limit: query.limit ?? 25 } }
      );
      if (!full_details && Array.isArray(data.events)) {
        return { ...data, events: data.events.map(compactEvent) };
      }
      return data;
    })
  );
  ...
  server.registerTool(
    ...
    handle(async (args: { category: string; include_product_metadata?: boolean; full_details?: boolean }) => {
      ...
      if (!full_details && Array.isArray(data.series)) {
        return { ...data, series: data.series.map(compactSeries) };
      }
      return data;
    })
  );
  ...
  server.registerTool(
    "kalshi_list_milestones",
    {
      ...
      inputSchema: {
        limit: z.number().int().min(1).max(500).optional().describe("Results per page (default 25)"),
        cursor: z.string().optional().describe("Pagination cursor from a previous response"),
    ...
    handle(async (args: Record<string, string | number | undefined>) =>
      client.request("GET", "/milestones", { query: { ...args, limit: args.limit ?? 25 } })
    )
  );
  ...
  server.registerTool(
    "kalshi_list_multivariate_collections",
    {
      ...
      inputSchema: {
        limit: z.number().int().min(1).max(200).optional().describe("Results per page (default 25)"),
        cursor: z.string().optional().describe("Pagination cursor from a previous response"),
    ...
    handle(async (args: Record<string, string | number | undefined>) =>
      client.request("GET", "/multivariate_event_collections/", { query: { ...args, limit: args.limit ?? 25 } })
    )
  );
